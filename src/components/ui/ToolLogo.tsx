 ...
 * anything unknown falls back to a neutral initials badge. All ~20px.
 */
...
function Badge({
  ...
  return (
    <span
      ...
      className={cn(
        box,
        "flex items-center justify-center rounded-[5px] text-[8.5px] leading-none font-bold",
      )}
      style={
        {
          background: bg,
...
function ReactMark() {
  return (
    <svg className={box} viewBox="-11.5 -10.23 23 20.46" aria-hidden>
...
function TailwindMark() {
  return (
    <svg className={box} viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#38bdf8"
...
export function ToolLogo({ name }: { name: string }) {
  ...
  if (n.includes("photoshop")) return <Badge bg="#001e36" fg="#31a8ff" label="Ps" />;
  if (n.includes("illustrator")) return <Badge bg="#330000" fg="#ff9a00" label="Ai" />;
  if (n.includes("after effects")) return <Badge bg="#00005b" fg="#9999ff" label="Ae" />;
  if (n.includes("premiere")) return <Badge bg="#2a0634" fg="#ea77ff" label="Pr" />;
  if (n.includes("powerpoint")) return <Badge bg="#c43e1c" fg="#ffffff" label="P" />;
  if (n.includes("excel")) return <Badge bg="#1d6f42" fg="#ffffff" label="X" />;
  if (n.includes("typescript")) return <Badge bg="#3178c6" fg="#ffffff" label="TS" />;
  if (n.includes("next")) return <Badge bg="#0a0a0a" fg="#ffffff" label="N" border />;
  if (n.includes("python")) return <Badge bg="#2b5b84" fg="#ffd343" label="Py" />;
  if (n.includes("figma")) return <FigmaMark />;
  if (n.includes("react")) return <ReactMark />;
  if (n.includes("tailwind")) return <TailwindMark />;
  if (n.includes("vs code") || n.includes("vscode")) return <VSCodeMark />;
  if (n.includes("claude")) return <ClaudeMark />;

  const initials =
    name.replace(/[^a-zA-Z0-9]/g, "").slice(0, 2).toUpperCase() || "•";
  return <Badge bg="#1a1a1e" fg="#8a7cff" label={initials} border />;
}
